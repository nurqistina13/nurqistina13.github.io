...
server <- function(input, output) {
  # Load the dataset
  mean_employment_2021 <- read.csv("mean_employment_2021.csv")  # Assuming the file is in the project directory
  
  output$line_plot <- renderPlotly({
    # Create a bar chart with plotly
    p <- plot_ly(data = mean_employment_2021, x = ~University, y = ~Mean.Employment.Rate.in.2021, type = 'bar') %>%
